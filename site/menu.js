...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.supabase && supabaseUrl && supabaseAnonKey) {
        ...
        // === Загрузка тарифов из локального API ===
        async function loadTariffsAPI() {
            ...
            if (list) {
                list.innerHTML = '';
                (data || []).filter(t => t.is_active).forEach((t) => {
                    const item = document.createElement('div');
                    item.className = 'bike-list-item tariff-option';
                    item.dataset.tariff = t.slug || '';
                    item.dataset.tariffId = t.id;
                    item.innerHTML = `<strong>${t.title}</strong><span>${t.price_rub} ₽ / ${t.duration_days} д.</span>`;
                    list.appendChild(item);
                });
            }
            // Обновляем глобальную карту тарифов
            // Build a map of tariffs keyed by slug or lowercase title. Include deposit and
            // parsed extensions so the main UI can offer custom durations and costs.
            const tariffMap = {};
            (data || []).forEach((t) => {
                let extensions = null;
                if (t.extensions) {
                    try {
                        // If extensions is already an object (from fallback localStorage) leave as is
                        extensions = typeof t.extensions === 'string' ? JSON.parse(t.extensions) : t.extensions;
                    } catch (e) {
                        extensions = null;
                    }
                }
                tariffMap[t.slug || t.title.toLowerCase()] = {
                    id: t.id,
                    cost: t.price_rub,
                    days: t.duration_days,
                    deposit: t.deposit_rub || 0,
        ...
        if (tariffBtn) {
            tariffBtn.addEventListener('click', loadTariffsAPI);
        }
    }
});
